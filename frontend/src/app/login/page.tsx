    ...
    <div style={{ display: "flex", height: "100vh", width: "100vw", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <h2 style={{ fontSize: "2.5rem", fontWeight: 700, lineHeight: 1.2, marginBottom: "20px", color: "#fff" }}>
            Desarrolla con metodología. <span style={{ color: "var(--primary)" }}>Construye con IA.</span>
          </h2>
          <p style={{ color: "var(--text-muted)", fontSize: "1.05rem", lineHeight: 1.6, marginBottom: "40px", maxWidth: "480px" }}>
            Instala en 1 comando. Tu agente de IA sigue el flujo SDD completo por ti.
          </p>

          {/* Features */}
          {[
            { icon: "fa-list-check", title: "Flujo SDD completo", desc: "Spec → Code → Test → Verify → Docs → Commit" },
            { icon: "fa-table-cells", title: "Tablero de tasks", desc: "Define tus tasks, el agente las ejecuta" },
            { icon: "fa-user-pen", title: "Man in the loop", desc: "Tú apruebas cada paso — nunca pierdes el control" },
          ].map(f => (
            <div key={f.icon} style={{ display: "flex", gap: "16px", alignItems: "flex-start", marginBottom: "24px" }}>
              <div style={{ width: "40px", height: "40px", borderRadius: "10px", background: "rgba(78,204,163,0.08)", border: "1px solid rgba(78,204,163,0.2)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      ...
      <div style={{ width: "480px", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px", background: "var(--bg-color)", flexShrink: 0 }}>
        <div style={{ marginBottom: "40px" }}>
          <h2 style={{ fontSize: "1.8rem", fontWeight: 700, marginBottom: "8px" }}>Bienvenido de vuelta builder 👋</h2>
          <p style={{ color: "var(--text-muted)" }}>Tu agente te está esperando</p>
        </div>
        ...
        <p style={{ marginTop: "32px", fontSize: "0.9rem", color: "var(--text-muted)", textAlign: "center" }}>
          ¿Eres nuevo? <Link href="/register" style={{ color: "var(--primary)", textDecoration: "none", fontWeight: 600 }}>Únete como builder →</Link>
        </p>
      </div>
    </div>
  );
